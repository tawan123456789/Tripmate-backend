...
async findOne(id: string, req: any) {
  if(req.user.id){const crc = await this.prisma.carRentalCenter.findUnique({ where: { id },include: { cars: true, service: {include: { reviews: true,location: true, bookmarks: {where : { userId: req.user.id} } } } } });
